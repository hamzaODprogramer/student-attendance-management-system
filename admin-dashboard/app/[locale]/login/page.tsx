    ...
    return <>
        <main className="flex h-full justify-center items-center">
            <div className="rounded-lg pt-6 pb-14 shadow-sm bg-white shadow-black/30 flex flex-col justify-center items-center w-[35%]">
                ...
                <h1 className="text-sm text-opacity-60 mt-1 font-semibold text-[#0a0a77]">{Login('Sign_into_your_account')}</h1>
                <div className="grid grid-cols-1 gap-y-5 pt-8">
                    <TextField className="w-[430px]" label={Login('Username')} type="text"/>
                    <FormControl className="w-[430px]" variant="outlined">
                    ...
                    <div className="flex items-center justify-between mt-[-20px]">
                        <FormControlLabel control={<Checkbox defaultChecked className="opacity-65"/>} className="opacity-65" label={Login('Remembre_Me')} />
                        <Link replace href={`/${lg}/login/reset`} className="text-[#0a0a77] text-opacity-70 text-md font-normal">{Login('Forgot_Password_?')}</Link>
                    </div>
                    <Button variant="contained" className="bg-[#0a0a77] font-[Poppins] py-2">{Login('login')}</Button>
                </div>
            </div>
        </main>
